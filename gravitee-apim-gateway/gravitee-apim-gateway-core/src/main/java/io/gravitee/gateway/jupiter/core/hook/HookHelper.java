...
public class HookHelper {
    ...
    public static <T> Maybe<T> hook(
        ...
        if (hooks != null && !hooks.isEmpty()) {
            return executeHooks(componentId, hooks, HookPhase.PRE, ctx, executionPhase, null, null)
                .andThen(maybe)
                .switchIfEmpty(executeHooks(componentId, hooks, HookPhase.POST, ctx, executionPhase, null, null).andThen(Maybe.empty()))
    ...
    private static <T extends Hook> Completable executeHooks(
        ...
        return Flowable
            ...
            .doOnError(error -> log.warn(String.format("Unable to execute '%s' hook on flow '%s'", phase.name(), componentId), error))
            .onErrorComplete();
    }
}
