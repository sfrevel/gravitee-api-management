 ...
 */
public class DebugRequestStep extends DebugStep<MutableRequest> {

    public DebugRequestStep(final String policyId, final ExecutionPhase executionPhase, final String flowPhase) {
        super(policyId, executionPhase, flowPhase);
    }

    @Override
    public Single<DebugStepState> saveInputState(final MutableRequest request, final Map<String, Serializable> inputAttributes) {
        return request
            .bodyOrEmpty()
            .doOnSuccess(buffer -> System.out.println("saveInputState bodyOrEmpty: " + policyId + " " + buffer))
            .map(inputBody ->
                new DebugStepState()
    ...
    public Single<Map<String, Object>> computeDiff(
        ...
        return request
            .bodyOrEmpty()
            .doOnSuccess(buffer -> System.out.println("computeDiff bodyOrEmpty: " + policyId + " " + buffer))
            .map(outputBody -> {
